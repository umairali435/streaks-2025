...
class PurchasesBloc extends Bloc<PurchasesEvent, PurchasesState> {
  ...
  Future<void> _onInit(InitPurchases event, Emitter<PurchasesState> emit) async {
  ...
  Future<void> _initSubscription(Emitter<PurchasesState> emit) async {
    ...
    if (Platform.isAndroid) {
      configuration = PurchasesConfiguration("goog_FVrtLHMkDutnKdFqQyNdlgAonGs");
    } else {
      configuration = PurchasesConfiguration("appl_KVYUCljoRyvHdMLJNmTqoYPmYQT");
    }
    ...
    if (emit.isDone) {
      emit(state.copyWith(isSubscriptionActive: info.activeSubscriptions.isNotEmpty));
    }
  }

  Future<void> _onFetchOffers(FetchOffers event, Emitter<PurchasesState> emit) async {
    try {
  ...
  Future<void> _onPurchase(PurchaseSubscription event, Emitter<PurchasesState> emit) async {
    emit(state.copyWith(isLoading: true));
    try {
  ...
  Future<void> _onRestore(RestoreSubscription event, Emitter<PurchasesState> emit) async {
    emit(state.copyWith(isLoading: true));
    try {
  ...
  Future<void> _onSelectPackage(SelectPackage event, Emitter<PurchasesState> emit) async {
    emit(state.copyWith(
